    ...
    if (params.name && params.surname && params.nick && params.email && params.password) {
        ...
        User.find({
            ...
            if (users && users.length >= 1) {
                ...
                //Cifrar la contraseña y guadar los datos
                bcrypt.hash(params.password, null, null, (err, hash) => {
                    user.password = hash;
                    user.save((err, userStored) => {
                        if (err) return res.status(500).send({ message: 'Error al guadar al usuario' })
...
function uploadImage(req,res){
    ...
    if(req.files){
        ...
        return res.status(200).send({ message: 'No se envío una imagen'});
